  ...
  return (
    <aside className="w-88 sticky bg-container/50 rounded-xl h-min max-h-full overflow-y-auto overflow-hidden">
      ...
      <nav className="p-4 -my-4 flex flex-col gap-3">
        ...
        <div
          className="p-2 rounded-md w-full max-w-full bg-slate-50 "
        >
          <p className="text-accent-violet text-sm font-bold text-pretty break-words w-full">{user.name} {' '}<span className="text-black">Snippet's</span></p>
        </div>

        {snippets.length > 0 ? (snippets.map((snippet: Snippet) => (
          <SnippetNavCard key={snippet._id} snippet={snippet} to={snippet._id}/>
        ))) : (
          <div className="font-normal text-sm p-2 rounded-md text-slate-50 text-center">
            <h3 className="text-sm font-bold text-pretty break-words w-full">El usuario aún no tiene Snippets</h3>
          </div>
        )}
