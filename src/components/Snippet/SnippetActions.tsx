...
export default function SnippetActions({ snippetId, isPending, isGuest }: SnippetActionsProps) {
  ...
  return (
    <Menu>
      <MenuButton
        className={
          'absolute right-5 top-3 outline-none data-active:text-purple-300 data-active:bg-container bg-container/80 hover:bg-container hover:text-purple-300  cursor-pointer group text-white rounded-lg transition-colors duration-300'
        }
      >
        <ChevronDoubleDownIcon
          strokeWidth={2}
          className={`size-8 p-1.5 transition-transform duration-300 rotate-0 group-data-active:rotate-180`}
        />
      </MenuButton>
      <MenuItems
        anchor="bottom"
        className="focus:outline-none z-50 mt-1 flex flex-col shadow-lg w-40 bg-container overflow-hidden rounded-lg"
      >
        <MenuItem>
          <button
            onClick={() => {
        ...
        <MenuItem>
          <button
            ...
            disabled={isPending}
          >
            {isPending ? 'Eliminado...' : 'Eliminar Snippet'}
          </button>
        </MenuItem>
      </MenuItems>
    </Menu>
